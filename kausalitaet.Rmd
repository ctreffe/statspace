...
p <- d %>% ggplot() + 
  ...
  labs(x = "Bildungsniveau in Jahren", 
       y = "Sozioökonomischer Status (z-Werte)") +
...
2.  ❓ **Zeitliche Abfolge**: Wir haben keine Daten über eine zeitliche Abfolge der Variablen vorliegen. Damit kann eine *umgekehrte Kausalrichtung* nicht ausgeschlossen werden. Genau genommen sind drei Szenarien denkbar:
    ...
    -   Bildungsniveau und sozioökonomischer Status stehen in einer wechselseitigen Ursache-Wirkungs-Beziehung zueinander, d.h. beide sind sowohl Ursache als auch Wirkung.
...
1.  Damit wir überzeugt sein können, dass ein bestimmter Effekt nicht vorliegt, müssen wir besonderes Augenmerk auf unsere Typ-2-Fehlerrate, also den Anteil falsch-negativer Ergebnisse richten.
...
-   *Nicht die statistische Rechnung, sondern das Studiendesign ,bestimmt, ob eine Analyse ein starker Test einer kausalen Theorie ist.*

    -   Eine Korrelation zwischen zwei Variablen ist *konsistent* mit einer kausalen Beziehung zwischen diese Variablen. Wenn keine Korrelation gefunden wird, spricht das gegen eine kausale Beziehung.
...
-   Auch in diesem Fall sollten Praktiker sich als Risikomanager:innen verstehen. Es treffen die gleichen Überlegungen zu, die in unserem Artikel zu [Konfundierung](https://ctreffe.github.io/r-space/konfundierung.html) geschildert wurden.
